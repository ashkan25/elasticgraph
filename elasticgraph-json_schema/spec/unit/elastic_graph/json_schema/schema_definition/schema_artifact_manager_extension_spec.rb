    ...
    module SchemaDefinition
      RSpec.describe SchemaArtifactManagerExtension do
        ...
        let(:deprecated_element_class) do
          ::Struct.new(:name, :description, :defined_at, keyword_init: true)
        end

        let(:defined_at_class) do
          ::Struct.new(:path, :lineno, keyword_init: true)
        end

        let(:missing_necessary_field_class) do
          ::Struct.new(:fully_qualified_path, :field_type, keyword_init: true)
        end
        ...
        before do
          allow(ElasticGraph::SchemaDefinition::JSONSchemaPruner).to receive(:prune) { |json_schema| json_schema }
        end
